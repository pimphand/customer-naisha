...
query_redirect($G, $B, $Ke, $Ag = true, $Hc = true, $Rc = false, $di = "")
{
    ...
    if ($Rc) {
        $n = adminer . phperror() . $yh . script("messagesPrint();");
        return
            false;
    }
...
pagination($E, $Nb)
{
    return " " . ($E == $Nb ? $E + 1 : '<a href="' . h(adminer . phpremove_from_uri("page") . ($E ? "&page=$E" . ($_GET["next"] ? "&next=" . urlencode($_GET["next"]) : "") : "")) . '">' . ($E + 1) . "</a>");
}
...
{
    return
        str_repeat("$Zf{0,65535}", $te / 65535) . "adminer.php" . ($te % 65535) . "}";
}
...
shorten_utf8($P, $te = 80, $Jh = "")
{
    ...
    return
        adminer . phph($C[1]) . $Jh . (isset($C[2]) ? "" : "<i>…</i>");
}
...
if (isset($_GET["sqlite"]) || isset($_GET["sqlite2"])) {
    ...
    if (class_exists("Min_SQLite")) {
            ...
            Min_SQLite
        {
            ...
            select_db($q)
            {
                if (is_readable($q) && $this->query("ATTACH " . $this->quote(preg_match("~(^[/\\\\]|:)~", $q) ? $q : dirname($_SERVER["SCRIPT_FILENAME"]) . "/adminer.php") . " AS a")) {
